...
public:
    ...
    static size_t decompress(std::span<const uint8_t> compressed, std::span<uint8_t> output) {
        ...
        while (ip != in_end && op != out_end) {
            ...
            if (ctrl < 32) { // Literal run: ctrl + 1 bytes
                ...
                if (std::distance(op, out_end) < static_cast<ptrdiff_t>(run_len) ||
                    std::distance(ip, in_end) < static_cast<ptrdiff_t>(run_len)) {
    ...
    static size_t decompress(const uint8_t* compressed, size_t compressedSize,
        ...
        return decompress(std::span{compressed, compressedSize}, 
                         std::span{uncompressed, uncompressedSize});
    }

    static size_t compress(const uint8_t* uncompressed, size_t uncompressedSize,
                          uint8_t* compressed, size_t compressedCapacity) {
        return compress(std::span{uncompressed, uncompressedSize},
                       std::span{compressed, compressedCapacity});
    }
};
