...
struct PointXYZRGB {
    ...
    PointXYZRGB(float x, float y, float z, uint8_t r, uint8_t g, uint8_t b) 
...
public:
    ...
    PointT& operator[](size_t idx) {
        return points[idx];
    }
    
    const PointT& operator[](size_t idx) const {
        return points[idx];
    }
...
private:
    ...
    Point3D getPosition(const Point3D& point) const {
        return point;
    }
    
    Point3D getPosition(const PointXYZRGB& point) const {
